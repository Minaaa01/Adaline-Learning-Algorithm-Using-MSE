...
def Get_MSE_Value():
    Userinfo.append(MSE_threshold_Value.get())


def Decision_train_test():
    if ([Userinfo[0], Userinfo[1]] == ['Adelie', 'Gentoo']) or (
            [Userinfo[0], Userinfo[1]] == ['Gentoo', 'Adelie']):
        return Data_Preprocessing.train_c1_c2_X.replace(0, -1), Data_Preprocessing.train_c1_c2_Y.replace(0,
                                                                                                         -1), Data_Preprocessing.test_c1_c2_X.replace(
            0, -1), Data_Preprocessing.test_c1_c2_Y.replace(0, -1)
        ...
        return Data_Preprocessing.train_c2_c3_X.replace(0, 1), Data_Preprocessing.train_c2_c3_Y.replace(0,
                                                                                                        1), Data_Preprocessing.test_c2_c3_X.replace(
            0, 1), Data_Preprocessing.test_c2_c3_Y.replace(0, 1)
...
i7 = Label(master, textvariable=MSE_threshold)
MSE_threshold.set("Enter MSE threshold")
i7.place(x=20, y=260)

MSE_threshold_Value = Entry(master)
MSE_threshold_Value.place(x=175, y=260)
MSE_threshold_Value.focus_set()
MSE_threshold_Button = Button(master, text="Enter", width=8, borderwidth=4, command=Get_MSE_Value)
MSE_threshold_Button.place(x=300, y=260)

Bias_Value = IntVar()
Check_Box = Checkbutton(master, text="Add bias or not", variable=Bias_Value, offvalue=0)
Check_Box.place(x=20, y=290)

Bias_Button = Button(master, text="Add", width=10, borderwidth=4, command=Get_Bias_Value)
Bias_Button.place(x=22, y=310)
