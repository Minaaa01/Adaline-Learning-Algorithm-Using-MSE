...
def trainPerceptron(X_train, Y_train, Eta, number_of_epochs, MSE_Threshold, Bias):
    X_train = np.array(X_train)
    #  convert the features list to array to get the net value for  each
    Y_train = np.array(Y_train)
    # Fit the scaler using available training data. For normalization
    # this means the training data will be used to estimate the minimum and maximum observable values
    ...
    for i in range(0, int(number_of_epochs)):
        results = []
        ...
        for j in range(0, len(X_train)):
            ...
            Net_Value = (np.dot(weights.T, X_train_[j])) + Bias
            # update the weights
            # (Y_train[j] - y) To calculate The loss L "Error"
            weights = weights + float(Eta) * (Y_train[j] - Net_Value) * X_train_[j]
        for k in range(len(X_train_)):
            Error_For_New_Weights = Y_train[k] - (np.dot(weights.T, X_train_[k]))
            results.append(np.power(Error_For_New_Weights, 2))
        Calculated_MSE = (sum(results) / len(results)) * 0.5
        if Calculated_MSE <= float(MSE_Threshold):
...
def testPerceptron(X_test, W, Bias):
    ...
    for i in range(len(X_test)):
        ...
        Net_Value = (np.dot(W.T, X_test_[i])) + Bias
